...
def main():
    ...
    parser.add_argument("--max_completion_length", type=int, default=60)
    ...
    # num_processes = args.num_proc if args.num_proc is not None else cpu_count()
    num_processes = 2
    ...
    if os.path.exists(args.trie_path):
        print(f"Loading pre-built Trie from {args.trie_path}...")
        with open(args.trie_path, "rb") as f_in:
            docid_trie = pickle.load(f_in)
        print("Trie loaded successfully.")
    else:
        print(f"Warning: Trie file not found at {args.trie_path}. Building Trie on-the-fly...")
        chunk_size = len(all_encoded) // num_processes + 1
        trie_sequences = [[tokenizer.pad_token_id] + item for item in all_encoded]
        # chunks = [trie_sequences[i:i + chunk_size] for i in range(0, len(trie_sequences), chunk_size)]
        # with Pool(num_processes) as p:
        #     partial_tries = list(tqdm(p.imap(build_partial_trie, chunks), total=len(chunks), desc="Building partial Tries"))
        # print("Merging partial Tries...")
        # final_trie = Trie()
        # for partial_trie_dict in tqdm(partial_tries, desc="Merging Tries"):
        #     final_trie.merge(partial_trie_dict)
        # docid_trie = final_trie
    ...
    def prefix_allowed_tokens_fn(batch_id, sent):
        ...
        if len(outputs) == 0:
            if(len(sent.tolist()) < 3):
                print(f"sent.tolist(): {sent.tolist()}")
            outputs = [tokenizer.pad_token_id]
        return outputs
    # def prefix_allowed_tokens_fn(batch_id, sent):
    ...
    #     # === Debug ===
    #     # if len(sent_list) == 1 and sent_list[0] == 0:
    #     #     # 如果是第一步，打印一下允许的长度
    #     #     print(f"[DEBUG] Step 1. Allowed count: {len(outputs)}")
    #     #     if 1 in outputs:
    #     #         print("[DEBUG] ALERT: 1 is in outputs!")
    #     # =============

    #     if len(outputs) == 0:
    #         outputs = [tokenizer.pad_token_id]
    #     if 1 in outputs and len(sent_list) == 1:
    #         print(f"[DEBUG] ALERT: 1 is in outputs for input {sent_list}!")
            
    #     if 0 in outputs and len(sent_list) < 3:
    #         print(f"[DEBUG] ALERT: 0 is in outputs for input {sent_list}!")
    #     return outputs
    ...
    trainer = CustomGRPOTrainer(
        original_to_encoded_list=original_to_encoded_list, # <--- 传入映射表
        ce_loss_weight=0.1,                                # <--- 设置 CE 权重 (推荐 0.1)
        rank_map_path=RANK_MAP_PATH,
        model=model,
        args=grpo_config,
        reward_funcs=[reward_scorer.reward_function],
        ...
        generation_kwargs={
            ...
            # "temperature": 1.3, # 增加随机性，鼓励探索 (可以尝试 0.7 - 1.0)
